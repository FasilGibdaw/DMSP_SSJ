...
class DMSPDataAnalyzer:
    def __init__(self):
        self.ch_energies = [30000., 20400., 13900.,  9450.,  6460.,  4400.,  3000.,  2040.,
                            1392.,   949.,   646.,   440.,   300.,   204.,   139.,    95.,
                            65.,    44.,    30.]
        self.index_labels = ['MLT', 'MLAT', 'ePrep', 'iPrep']

    def dmsp_reader(self, fname, channel=1):
        """_summary_
    ...
    def round_off(self, number, reciprocal=2):
    ...
    def dmsp_grid(self, D, reciprocal):
        """_summary_
        ...
        """
        df = pd.DataFrame(D, columns=self.index_labels)
        df = df.apply(self.round_off, args=[reciprocal])
    ...
    def dmsp_polar_plot(self, x, y, egrided_data, igrided_data, ch=1, savefig=False):
        """This plots the ion and electron energy on a specific channel (ch) on a given grid 

        Args:
            x (float): grided mlt
            y (float): grided magentic latitude
            egrided_data (array): electrons energy on the grid (from channel ch)
            igrided_data (array): ions energy on the grid (from channel ch)
        ...
        ax1.text(0.85, 0.95, str(
            self.ch_energies[ch]/1000) + 'KeV', transform=ax1.transAxes)
        ...
        ax2.text(0.85, 0.95, str(
            self.ch_energies[ch]/1000) + 'KeV', transform=ax2.transAxes)
        fig.colorbar(c, label='Log10 (Proton diff. energy flux)')
        if savefig == True:
            plt.savefig(
                'DMSP_at_'+str(self.ch_energies[ch]/1000) + 'KeV'+'.png', dpi=800)
        #plt.show()

    @staticmethod
    def main():
        instance = DMSPDataAnalyzer()
        fname = './data/dmsp-f18_ssj_precipitating-electrons-ions_20120101_v1.1.1.cdf'
        ch = 5
        try:
            D = instance.dmsp_reader(fname, channel=ch)
            x, y, egrided_data, igrided_data = instance.dmsp_grid(D, 2)
            instance.dmsp_polar_plot(x, y, egrided_data, igrided_data, ch=ch, savefig=True)
        except:
